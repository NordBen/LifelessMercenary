...
public class FileDataHandler : MonoBehaviour
{
    ...
    public SaveGameData Load(string profileId, bool allowRestoreFromBackup = true) 
    {
        ...
        if (File.Exists(fullPath)) 
        {
            try 
            {
                ...
                loadedData = JsonUtility.FromJson<SaveGameData>(dataToLoad);
            }
            catch (Exception e) 
            {
    ...
    public void Save(SaveGameData data, string profileId) 
    {
        Debug.Log($"trying to Save {data} with profile {profileId}");
        // base case - if the profileId is null, return right away
        if (profileId == null) 
        {
            return;
        ...
        Debug.LogWarning($"fullP {fullPath} with backup path {backupFilePath}");
        try 
        {
            ...
            SaveGameData verifiedGameData = Load(profileId);
        ...
        Debug.LogError($"Filepath: {fullPath}");
    }
    ...
    public Dictionary<string, SaveGameData> LoadAllProfiles() 
    {
        Dictionary<string, SaveGameData> profileDictionary = new Dictionary<string, SaveGameData>();
        ...
        foreach (DirectoryInfo dirInfo in dirInfos) 
        {
            ...
            SaveGameData profileData = Load(profileId);
    ...
    public string GetMostRecentlyUpdatedProfileId() 
    {
        ...
        Dictionary<string, SaveGameData> profilesGameData = LoadAllProfiles();
        foreach (KeyValuePair<string, SaveGameData> pair in profilesGameData) 
        {
            string profileId = pair.Key;
            SaveGameData gameData = pair.Value;
