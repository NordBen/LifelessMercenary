...
namespace LM.AbilitySystem
{
    ...
    public class GameplayAttribute : ScriptableObject
    {
        ...
        [SerializeField] public List<AttributeModification> _activeModifications = new();
        [SerializeField] public List<GameplayEffectApplication> _activeEffectApplications = new();
        ...
        {
            Debug.Log(
                $"activeEffectApplications: {_activeEffectApplications.Count} and activeEffectApplications: {_activeModifications.Count} when SettingValue for {this._name}");
            LogAllActiveEffectsForAttribute();
        ...
        private void LogAllActiveEffectsForAttribute()
        {
            foreach (var app in _activeEffectApplications)
            {
                Debug.Log(
                    $"found application {app} for {this._name} with values modifierType of - {app.modifierOperation}, target of -{app.targetAttribute._name}, strategy of - {app.valueStrategy} with {app.GetComputedValue()}");
            }

            foreach (var mod in _activeModifications)
            {
                Debug.Log(
                    $"applying {mod} with {mod.effect} and {mod.value} with applications: {mod.effect.applications.Count} and execs: {mod.effect.executions.Count} for {this._name}");
            }
        }
        /*
        ...
        public void RecalculateCurrentValue()
        {
            ...
            {
                float before = finalValue;
                finalValue = CalculateModifiedValue(finalValue, modification.GetComputedValue(),
                    modification.modifierOperation);
